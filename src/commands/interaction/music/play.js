...
module.exports = {
    ...
    options: [
        {
            name: "query",
            description: "Specify the song title or URL",
            type: 3,
            required: true,
        },
    ],
    ...
    run: async (client, interaction, player) => {
        ...
        if (result.type === "EMPTY" || result.type === "ERROR" || !result.tracks.length) {
            embed.setDescription(`Nothing was found for your request.`);
            return interaction.editReply({ embeds: [embed], ephemeral: true });
        }
        ...
        if (result.type === "PLAYLIST") {
            ...
            embed.setDescription(`Added playlist **[${result.playlistName}](${query})** - \`${result.tracks.length}\` songs to the queue.`);
        } else {
            ...
            embed.setDescription(`Added song **[${trackTitle} - ${trackAuthor}](${track.uri})** - \`${convertTime(track.duration)}\`.`);
        }
